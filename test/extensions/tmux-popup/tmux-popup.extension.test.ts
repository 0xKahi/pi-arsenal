    ...
    mock.module('../../../src/config/config-loader', () => ({
      ConfigLoader: {
        load: (): ConfigLoadResult => ({
          success: true,
          config: { tmux_popup: { enabled: false, width: 50, height: 50, fileCommand: 'nvim' }, p2p_hub: { enabled: false } },
        }),
      },
    }));
  ...
  it('registers the tool when enabled', () => {
    mock.module('../../../src/config/config-loader', () => ({
      ConfigLoader: {
        load: (): ConfigLoadResult => ({
          success: true,
          config: { tmux_popup: { enabled: true, width: 50, height: 50, fileCommand: 'nvim' }, p2p_hub: { enabled: false } },
        }),
      },
    }));
