...
import {
  ...
  formatRemotePrompt,
  renderP2pHubMessage,
} from '../../../src/extensions/p2p-hub/communication-presentation';
...
describe('p2p communication presentation', () => {
  ...
  test('message renderer distinguishes steers, batches, and remote prompts', () => {
    const steer = renderP2pHubMessage(
    ...
    expect(steerText).toContain('peer message · steer');
    expect(steerText).toContain('@alpha');
    expect(steerText).toContain('hello');

    const batch = renderP2pHubMessage(
      {
        content: 'model batch',
        details: {
          kind: 'batch',
    ...
    expect(batchText).toContain('2 peer messages · triggered when idle');
    ...
    const prompt = renderP2pHubMessage(
      {
    ...
    expect(promptText).toContain('remote prompt');
  ...
  test('message renderer never exceeds narrow widths', () => {
    const component = renderP2pHubMessage(
      {
        content: 'fallback',
        details: {
          kind: 'batch',
