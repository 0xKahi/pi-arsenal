...
describe('P2pCouncilState', () => {
  ...
  test('createCouncil starts a host and writes a live registry entry', async () => {
    const host = spawn('host-a');
    const result = await host.createCouncil('team-a');
  ...
  test('createCouncil rejects a name with a live registry entry', async () => {
    const host = spawn('host-a');
    await host.createCouncil('dup-council');

    const other = spawn('host-b');
    const result = await other.createCouncil('dup-council');
    expect(result.success).toBe(false);
  });

  test('client joins a council and both sides see the roster', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-b');
    const entry = registry.read('team-b');
    expect(entry).toBeDefined();
    if (!entry) return;

    const client = spawn('client-a');
    const join = await client.joinCouncil(entry);
  ...
  test('canonical model IDs survive registration, welcome snapshots, status updates, roster access, and peeks', async () => {
    ...
    await host.createCouncil('model-council');
    const entry = registry.read('model-council');
  ...
  test('a joining client immediately receives the host and all existing clients with explicit connection types', async () => {
    const host = spawn('host-a');
    await host.createCouncil('topology-council');
    const entry = registry.read('topology-council');
    if (!entry) throw new Error('missing entry');
    const clientA = spawn('client-a');
    await clientA.joinCouncil(entry);
    const clientB = spawn('client-b');
    const result = await clientB.joinCouncil(entry);
  ...
  test('join fails cleanly when an open transport never sends welcome', async () => {
    ...
    expect(client.getCouncilName()).toBeUndefined();
    expect(client.getRoster()).toEqual([]);
    await new Promise<void>(resolve => server.close(() => resolve()));
  }, 5000);

  test('council deduplicates a colliding member name', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-c');
    const entry = registry.read('team-c');
    if (!entry) throw new Error('missing entry');

    const client1 = spawn('same-name');
    await client1.joinCouncil(entry);
    const client2 = spawn('same-name');
    await client2.joinCouncil(entry);
    await Bun.sleep(20);
  ...
  test('chat with triggerTurn:false delivers as a steer message on the target', async () => {
    ...
    await host.createCouncil('team-e');
  ...
  test('chat targeting an unknown member fails with not_found', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-f');
  ...
  test('askPrompt round-trips through prompt_request/prompt_response', async () => {
    ...
    await host.createCouncil('team-g');
  ...
  test('peek returns the roster without registering the peeker as a member', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-i');
    ...
    expect(snapshot?.councilName).toBe('team-i');
  ...
  test('manual disconnect removes the registry entry when no clients remain', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-j');
  ...
  test('member_left fires when a client disconnects, and roster updates on the host', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-k');
  ...
  test('runtime replacement rejects stale callbacks and refreshes model/context status', async () => {
    ...
    await host.createCouncil('binding-council');
  ...
  test('queues trigger-turn and steer chat while detached and rejects prompts temporarily', async () => {
    ...
    await host.createCouncil('detached-council');
    const entry = registry.read('detached-council');
    if (!entry) throw new Error('missing entry');
  ...
  test('detach terminates outgoing and incoming runtime-owned prompt work', async () => {
    ...
    await host.createCouncil('prompt-detach-council');
    const entry = registry.read('prompt-detach-council');
    if (!entry) throw new Error('missing entry');
  ...
  test('handoff timeout disposes transport, registry ownership, and retained queues', async () => {
    ...
    await host.createCouncil('timeout-council');
    ...
    try {
      ...
      expect(registry.exists('timeout-council')).toBe(false);
    } finally {
      jest.useRealTimers();
    }
  });
  ...
  test('promotion: a client rebinds the same port and becomes host after the original host disconnects', async () => {
    const host = spawn('host-a');
    await host.createCouncil('team-l');
