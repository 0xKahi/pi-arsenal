...
The system SHALL provide a `p2p_ls` tool that lists the connected hub's name and routing-relevant member information. Human-readable and structured member output SHALL include each member's name, whether it is the calling session, status when known, cwd when known, and description when set. The tool output SHALL omit model, context usage, context progress bars, and network connection type; those diagnostics remain available through the persistent hub widget and hub detail UI.

#### Scenario: Listing members
- **WHEN** `p2p_ls` is called while connected to a hub with multiple members
- **THEN** the result identifies the hub and lists each member's name, self marker, status, description when set, and cwd when known
