...
Defines the hub-and-spoke networking layer for pi-to-pi communication: named council lifecycle, discovery via a per-user registry, agent identity, membership, host-death promotion, and the peek roster query.

## Requirements

### Requirement: Named councils as independent localhost servers

The system SHALL support multiple concurrently running councils, each identified by a unique name and served on its own `127.0.0.1` port. Agents connected to one council SHALL NOT be visible to or reachable from agents on another council.

#### Scenario: Two councils are isolated
- **WHEN** agent A is connected to council "frontend" and agent B is connected to council "infra"
- **THEN** neither agent appears in the other's member list and messages cannot be exchanged between them

#### Scenario: Creating a council
- **WHEN** an agent creates a council with a name not present in the registry
- **THEN** a server is started on an available localhost port, the creating agent becomes the council host, and a registry entry is written for the council

#### Scenario: Creating a council whose name already exists
- **WHEN** an agent attempts to create a council with a name that has a live registry entry
- **THEN** the creation is rejected with an error identifying the existing council

### Requirement: Council registry with staleness detection

The system SHALL maintain a per-user registry of councils, one file per council at `~/.arsenal/p2p-councils/<name>.json`, recording at least the council name, port, host process id, and creation time. Registry writes SHALL be atomic. Before presenting or connecting to a registered council, the system SHALL validate the entry (host process liveness and/or connect probe) and SHALL prune entries that fail validation.

#### Scenario: Stale entry pruned
- **WHEN** a registry entry's host process is dead and no server answers on the recorded port
- **THEN** the entry is removed and the council is not listed as available

#### Scenario: Clean shutdown removes the entry
- **WHEN** a council host disconnects intentionally and no clients remain to promote
- **THEN** the council's registry entry is removed

#### Scenario: Legacy registry is not discovered
- **WHEN** `~/.arsenal/p2p-hubs/` contains entries and `~/.arsenal/p2p-councils/` contains none
- **THEN** the legacy entries are neither listed nor migrated as available councils

### Requirement: Agent identity resolution

An agent's identity SHALL consist of a name, an optional description, its canonical Pi model ID, its cwd, its status, and its context usage. The canonical model ID SHALL be sourced from the active model's `id` value rather than its human-readable display name. Name and description SHALL be read from `<cwd>/.arsenal/p2p-role.yml` (keys `name`, `description`) where cwd is the directory pi was launched from, with no upward directory traversal. When the file is absent or omits `name`, the name SHALL default to the basename of the cwd. The council SHALL deduplicate colliding names by appending a numeric suffix.
...
Members of a council SHALL maintain an explicit topology that identifies exactly one host connection type and zero or more client connection types, independently of whether a member is the local session. A joining client's welcome handshake SHALL explicitly identify the host and existing clients and carry their identities and statuses. A join SHALL report success only after that welcome handshake has assigned the joiner's final name and initialized its complete roster. Members SHALL subsequently receive join/leave notifications and status updates for all other members. Status updates SHALL carry the member's current activity state (idle, thinking, or the active tool name), canonical model ID, and context usage (tokens and context window), so consumers can render usage without additional queries.

#### Scenario: Join visibility
- **WHEN** a new agent joins a council with an existing host and clients
...
- **THEN** its local roster classifies the remote council owner with connection type `host` and itself and all other non-host members with connection type `client` while separately identifying itself as the local member
...
When the council host terminates without a clean council shutdown, remaining clients SHALL race, after a jittered delay, to bind the council's recorded port. Exactly one client SHALL win (port binding is the arbiter), become the new host, and atomically update the registry entry with its own process id while preserving the council name and port. Losing clients SHALL reconnect to the new host as clients. The council SHALL remain addressable under the same name and port across promotion.

#### Scenario: Promotion after host crash
- **WHEN** a council host process dies while two clients are connected
...
- **WHEN** a client disconnects from a council intentionally
- **THEN** it does not attempt to rejoin or promote, and remaining members continue undisturbed

### Requirement: Peek roster query

The system SHALL support a peek operation: a short-lived connection to a council that requests the current roster (host and clients with full identity: name, model, context usage, status, description, cwd) without registering as a member. Peekers SHALL NOT appear in any member list, status update, or member-visible notification.

#### Scenario: Peek returns roster without joining
- **WHEN** an agent peeks a council it is not connected to
- **THEN** it receives the council's current roster and the council's members observe no join or leave event

### Requirement: Process-scoped connection across runtime replacement

A connected council membership SHALL belong to the running Pi process rather than to one replaceable extension runtime. When Pi replaces the extension runtime for reload, new, resume, or fork while retaining the process, the system SHALL preserve the same transport connection, council name, host/client connection type, assigned member name, roster, and queued inbound chat messages. The replacement runtime SHALL bind to that connection without producing a member leave/join cycle or creating a second local member. Final Pi process shutdown SHALL close the preserved connection using the existing clean shutdown behavior.

#### Scenario: Hosted council survives reload
- **WHEN** a Pi process hosting a council replaces its extension runtime for `/reload`
...
- **THEN** the replacement runtime reports the client connected under the same assigned name and client connection type, and the council observes no leave or duplicate join
...
- **WHEN** a council member sends chat while the connected process has no attached extension runtime during replacement
- **THEN** the message remains queued and is delivered once the replacement runtime binds, without creating a duplicate council member
