...
The extension SHALL register the `tmux_popup` tool only when the resolved `tmux_popup.enabled` value is `true`. A disabled tool SHALL NOT appear among Pi's available tools.
