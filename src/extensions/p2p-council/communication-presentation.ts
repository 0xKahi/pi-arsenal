...
function headingFor(details: P2pInboundDetails): string {
  if (details.kind === 'batch') {
    ...
    return `${details.items.length} council ${noun} · p2p_send · triggered when idle`;
  }
  if (details.kind === 'steer') return 'council message · p2p_send · steer';
  return 'council request · p2p_ask';
}
...
export function renderP2pCouncilMessage(
  ...
  const box = new Box(options.outputPad, 0, value => theme.bg('customMessageBg', value));
  box.addChild(new Text(theme.fg('customMessageLabel', theme.bold(headingFor(details))), 0, 1));
