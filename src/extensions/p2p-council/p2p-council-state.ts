...
export class P2pCouncilState {
  ...
  public disconnect(reason: 'manual' | 'disabled'): void {
    ...
    if (reason === 'manual' && priorCouncilName) {
      this.runtimeFor()?.notify(`disconnected from chambers: "${priorCouncilName}"`, 'info');
    }
  }
  ...
  private handleHostConnection(clientWs: WebSocket): void {
    ...
    clientWs.on('message', raw => {
      ...
      if (msg.type === 'register') {
        ...
        this.runtimeFor()?.notify(`member has joined: "${clientName}" -> "${this.councilName}"`, 'info');
        this.emitChange();
        return;
      }
    ...
    clientWs.on('close', () => {
      ...
      this.runtimeFor()?.notify(`"member has left: "${name}"`, 'info');
      this.emitChange();
    });
