...
export class P2PWidgetController {
  ...
  static renderWidget(ui: ExtensionUIContext, state: P2pCouncilState) {
  ...
  static renderLines(state: P2pCouncilState, theme: Theme, width: number): string[] {
    const bounded = safeWidth(width);
    ...
    return [
      border(bounded, borderChar('topLeft'), borderChar('topRight'), `${borderChar('horizontal')} ${dye.colorize(councilName, { fg: 'green' })} `),
