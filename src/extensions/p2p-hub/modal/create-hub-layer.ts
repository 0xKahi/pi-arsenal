...
/** Text-focused layer that creates a hub without leaving the custom modal. */
export class CreateHubLayer implements ModalLayer {
  ...
  public render(width: number, height: number | undefined): string[] {
    const lines = [this.theme.fg('accent', 'Create New Hub'), '', ...this.input.render(width)];
    if (this.busy) lines.push('', this.theme.fg('muted', 'Creating hub...'));
  ...
  private async submit(): Promise<void> {
    ...
    if (!name) {
      this.error = 'Hub name is required.';
      this.tui.requestRender();
      return;
    }
    if (name.includes('/') || name.includes('\\') || name.includes('\0')) {
      this.error = 'Hub name cannot contain path separators.';
      this.tui.requestRender();
      return;
    }
    ...
    this.error = undefined;
    this.tui.requestRender();
    const result = await this.state.createHub(name);
