...
export class HubRegistry {
  ...
  public constructor(dir: FileSearchResult = PathUtil.findArsenalConfig({ type: 'global', path: 'p2p-hubs' })) {
    this.dir = dir;
  }
  ...
  public read(name: string): HubRegistryEntry | undefined {
    ...
    try {
      ...
      if (isHubRegistryEntry(parsed)) return parsed;
      return undefined;
    } catch {
      return undefined;
    }
  ...
  public async write(entry: HubRegistryEntry): Promise<void> {
...
function isHubRegistryEntry(value: unknown): value is HubRegistryEntry {
...
export function probeHubPort(port: number, timeoutMs = PROBE_TIMEOUT_MS): Promise<boolean> {
 ...
 * (cheap), then fall back to a connect probe. Returns true when the hub
 * appears reachable.
 */
export async function isEntryLive(entry: HubRegistryEntry): Promise<boolean> {
  if (!isPidAlive(entry.hostPid)) return false;
  return probeHubPort(entry.port);
}

/** List only entries that pass liveness validation, pruning stale ones from disk. */
export async function listLiveHubs(registry: HubRegistry): Promise<HubRegistryEntry[]> {
  const all = registry.list();
  const live: HubRegistryEntry[] = [];
